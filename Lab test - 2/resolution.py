...
def resolve(kb, query):
    ...
    for rule in temp:
        steps[rule] = 'Given.'
    steps[negation(query)] = 'Negated query.'
    i = 0
    while i < len(temp):
        n = len(temp)
        j = (i + 1) % n
                ...
                if negation(c) in terms2:
                    ...
                    if len(gen) == 2:
                        ...
                        if contradiction(query,f'{terms1[0]}v{terms2[0]}'):
                            ...
                            steps[''] = f"Resolved {temp[i]} and {temp[j]} to {temp[-1]}, which is equal to null. \
                            \nThe final result after simplifying is null hence the query enatils the knowledgebase"
                            return steps
